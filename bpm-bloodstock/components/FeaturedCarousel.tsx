...
// Uses the photo when present, falling back to a branded green card.
    ...
    <div className="rounded-2xl border border-green-600 bg-green-800 p-5 md:p-8">
      <div className="grid items-center gap-8 md:grid-cols-2">
        ...
        <div>
          ...
          <h3 className="mt-4 font-heading text-4xl font-bold tracking-tight text-cream">
            {o.name}
          </h3>
          ...
          <p className="mt-4 text-cream/85">{o.tagline}</p>

          <ul className="mt-5 space-y-2.5">
            {perks.map((p) => (
              <li key={p} className="flex items-start gap-3 text-sm text-cream/85">
                <span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full bg-green-900 text-[11px] text-gold">
                  ✓
                </span>
                {p}
              </li>
