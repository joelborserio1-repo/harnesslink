...
<nav class="hld-cat-nav" aria-label="Directory categories">
  <div class="hld-cat-nav__scroll">
    <?php foreach ( $nav_types as $slug => $type ):
      $count    = isset( $counts[ $slug ] ) ? (int) $counts[ $slug ] : 0;
      $is_active= ( $slug === $active );
      $is_soon  = ( $count === 0 );
      $url      = hld_directory_url( $slug );
    ?>
      <a href="<?= esc_url( $url ) ?>"
         class="hld-cat-chip<?= $is_active ? ' hld-cat-chip--active' : '' ?><?= $is_soon ? ' hld-cat-chip--soon' : '' ?>"
         data-type="<?= esc_attr( $slug ) ?>"
         <?= $is_active ? 'aria-current="page"' : '' ?>>
        <span class="hld-cat-chip__icon"><?= esc_html( $type['icon'] ) ?></span>
        <span class="hld-cat-chip__label"><?= wp_kses( $type['plural'], array() ) ?></span>
        <?php if ( $is_soon ): ?>
